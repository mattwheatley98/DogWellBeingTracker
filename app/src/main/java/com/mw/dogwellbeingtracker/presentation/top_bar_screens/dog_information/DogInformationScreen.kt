...
import com.mw.dogwellbeingtracker.presentation.common.DogSexField
import com.mw.dogwellbeingtracker.presentation.common.DogTextField
 ...
 * Checks if the list of dogs is empty, and if it is, [EmptyListAnimationDogInformation] displays and prompts the user to add a dog.
 */
@Composable
private fun DogInformationBody(
    ...
    if (dogList.isEmpty()) { //Already initialized... don't need the loading state, unlike on the HomeScreen
        EmptyListAnimationDogInformation(addDog = addDog)
    } else {
        DogInformationList(
            dogList = dogList,
            deleteDog = deleteDog,
...
/**
 * Displays when the already-initialized list of dogs is empty.
 */
@Composable
private fun EmptyListAnimationDogInformation(addDog: () -> Unit, modifier: Modifier = Modifier) {
    val visibleState =
        remember {
            MutableTransitionState(initialState = false).apply {
                targetState = true
        ...
        Column(
            ...
            Card(modifier = modifier.padding(16.dp)) {
                Column(
                    ...
                    Text(
                        text = "No dogs found! Please add a dog using the below button:",
